...
bank_marketing_pca=bank_marketing[,c(1,11:14,16:20)]
str(bank_marketing_pca)

summary(bank_marketing_pca)
cor(bank_marketing_pca)
plot(cor(bank_marketing_pca))

# Using prcomp to compute the principal components (eigenvalues and eigenvectors). With scale=TRUE, variable means are set to zero, and variances set to one

bank_pca <- prcomp(bank_pca,scale=TRUE)
plot(bank_pca)
summary(bank_pca)
